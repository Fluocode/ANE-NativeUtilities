//
//  NativeDialogControler.m
//  NativeDialogs
//
//  Native dialog controller for the AIR Native Extension (alerts, lists, pickers, progress, text input).
//
...
#define FRE_ERROR (const uint8_t*)"error"
...
-(NSString*)getDisplayCutout{
    NSMutableDictionary *cutoutDict = [NSMutableDictionary dictionary];
    
    // Get the main window and its safe area insets
    UIWindow *window = nil;
    ...
    if (window && window.rootViewController && window.rootViewController.view) {
        ...
        if (@available(iOS 11.0, *)) {
            ...
            [cutoutDict setObject:[NSNumber numberWithFloat:safeInsets.top] forKey:@"top"];
            [cutoutDict setObject:[NSNumber numberWithFloat:safeInsets.left] forKey:@"left"];
            [cutoutDict setObject:[NSNumber numberWithFloat:safeInsets.right] forKey:@"right"];
            [cutoutDict setObject:[NSNumber numberWithFloat:safeInsets.bottom] forKey:@"bottom"];
            
            // For iOS, we typically don't have bounding rects like Android, but we can add empty array
            [cutoutDict setObject:[NSArray array] forKey:@"boundingRects"];
        } else {
            // iOS < 11, no safe area support
            [cutoutDict setObject:[NSNumber numberWithInt:0] forKey:@"top"];
            [cutoutDict setObject:[NSNumber numberWithInt:0] forKey:@"left"];
            [cutoutDict setObject:[NSNumber numberWithInt:0] forKey:@"right"];
            [cutoutDict setObject:[NSNumber numberWithInt:0] forKey:@"bottom"];
            [cutoutDict setObject:[NSArray array] forKey:@"boundingRects"];
        }
    } else {
    ...
    NSError *jsonError;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:cutoutDict options:0 error:&jsonError];
    if (jsonData) {
...
-(BOOL)setDisplayMode:(NSString*)displayMode withLayoutMode:(NSString*)layoutMode{
    // iOS doesn't have the same display mode concepts as Android
    // But we can handle fullscreen mode
    UIWindow *window = nil;
    if (@available(iOS 13.0, *)) {
        for (UIWindowScene *windowScene in [UIApplication sharedApplication].connectedScenes) {
            if (windowScene.activationState == UISceneActivationStateForegroundActive) {
    ...
    if (window && window.rootViewController) {
        if ([displayMode isEqualToString:@"fullscreen"]) {
            // iOS handles fullscreen through stage.displayState in ActionScript
            // This is mainly for consistency with Android API
            return YES;
        } else if ([displayMode isEqualToString:@"normal"]) {
            return YES;
        }
    }
...
-(void)blockScreenshot:(BOOL)block{
    // iOS doesn't have a direct API like Android's FLAG_SECURE
    // We use a combination of techniques:
    // 1. Show an overlay view when app goes to background (which also happens during screenshots)
    // 2. Detect screenshot attempts and hide sensitive content
    ...
    if (block) {
        ...
        if (!self.screenshotBlockerView) {
            CGRect screenRect = [[UIScreen mainScreen] bounds];
            self.screenshotBlockerView = [[UIView alloc] initWithFrame:screenRect];
            self.screenshotBlockerView.backgroundColor = [UIColor blackColor];
            self.screenshotBlockerView.alpha = 1.0;
            self.screenshotBlockerView.hidden = YES;
            self.screenshotBlockerView.userInteractionEnabled = NO;
            // Make it appear on top of everything
            self.screenshotBlockerView.layer.zPosition = CGFLOAT_MAX;
        }
        
        // Add to window if not already added
        if (self.screenshotBlockerView.superview != window) {
            [window addSubview:self.screenshotBlockerView];
            [window bringSubviewToFront:self.screenshotBlockerView];
        }
        ...
        // Also listen for screenshot notifications (iOS 7+)
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(userDidTakeScreenshot:)
                                                     name:UIApplicationUserDidTakeScreenshotNotification
                                                   object:nil];
    } else {
        // Remove overlay and observers
        if (self.screenshotBlockerView) {
            [self.screenshotBlockerView removeFromSuperview];
        }
...
-(void)applicationWillResignActive:(NSNotification *)notification{
    // Show overlay when app goes to background (this also happens during screenshots)
    if (self.screenshotBlockerView && self.screenshotBlockerView.superview) {
        self.screenshotBlockerView.hidden = NO;
        [self.screenshotBlockerView.superview bringSubviewToFront:self.screenshotBlockerView];
    }
}

-(void)applicationDidBecomeActive:(NSNotification *)notification{
    // Hide overlay when app becomes active again
    // Add a small delay to ensure screenshot is captured with black screen
...
-(void)userDidTakeScreenshot:(NSNotification *)notification{
    // Screenshot was taken - hide overlay after a moment
    // The overlay should have been visible during the screenshot
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if (self.screenshotBlockerView) {
            self.screenshotBlockerView.hidden = YES;
        }
...
-(NSString*)getDeviceUniqueId{
    // Use identifierForVendor (IDFV) - the only publicly available unique identifier in iOS
    // This ID is unique per vendor (app developer) and per device
    // It persists across app installs unless all apps from the same vendor are uninstalled
    ...
    // Fallback for iOS < 6.0 (very rare nowadays)
    // Store a UUID in user defaults
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSString *storedId = [defaults stringForKey:@"device_unique_id"];
    
    if (!storedId) {
        // Generate UUID
        CFUUIDRef uuid = CFUUIDCreate(kCFAllocatorDefault);
        CFStringRef uuidString = CFUUIDCreateString(kCFAllocatorDefault, uuid);
        // non-ARC: avoid __bridge_transfer and explicitly release CF object
        storedId = [NSString stringWithString:(__bridge NSString *)uuidString];
        CFRelease(uuid);
        CFRelease(uuidString);
        
        [defaults setObject:storedId forKey:@"device_unique_id"];
...
-(void)presentView:(UIView*)viewToPresent withToolBarItems:(FREObject *)toolbarItems andTitle:(NSString*)title{
    ...
    // iPad: present content in a modal custom style; iPhone uses the branch below.
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad)
    {
        ...
        // Optional: dim popover chrome (left commented; uses semi-transparent blackBG instead).
        ...
        // Catalyst / macOS: optional dark style hint via NSUserDefaults (may be unset on pure iOS).
    ...
    NSLog(@"presentView ::: 3" );
     ...
     NSLog(@"presentView ::: 4" );
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [ rootWindow presentViewController:popoverContent animated:NO completion:nil];
        ...
        if (isNotOS_8) {
            ...
            if(!wind){
                NSLog(@"Window is nil");
                FREDispatchStatusEventAsync(freContext, FRE_ERROR, (const uint8_t*)"Window is nil");
                return;
            }
            NSLog(@"presentView ::: 9" );
            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)actionSheetButtonClicked:(UIBarButtonItem*)sender {
    ...
    if(popover){
       ...
       NSLog(@"actionSheetButtonClicked");
        ...
        // Dismiss without animation so the popover teardown stays in sync with Flash events.
        [rootWindow dismissViewControllerAnimated:NO completion:nil];
    }else{
...
-(void)showPickerWithOptions:(FREObject*)options
    ...
    @catch (NSException *exception) {
        FREDispatchStatusEventAsync(freContext, FRE_ERROR, (const uint8_t *)[[exception reason] UTF8String]);
    }
#ifdef MYDEBUG
