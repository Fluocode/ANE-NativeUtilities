 ...
 * NativeDialogs
 *
 * AIR Native Extension: FRE entry points for native dialogs and utilities.
...
FREObject setBrightness(FREContext ctx, void* functionData, uint32_t argc, FREObject argv[] ){
    ...
    // -1 means "use user's preference" (Android doc). On iOS we treat it as no-op.
    if (value < 0.0) {
        return NULL;
    }
    // iOS expects brightness in range [0..1]. Some code may pass [0..100] or [0..255].
    if (value > 1.0) {
        if (value <= 100.0) {
            value = value / 100.0;
        } else {
...
FREObject blockScreenshot(FREContext ctx, void* functionData, uint32_t argc, FREObject argv[] ){
    NativeDialogControler *nativeDialogController = functionData;
    uint32_t block = 0;
    if (argc > 0 && argv[0]) {
        FREGetObjectAsBool(argv[0], &block);
    }
    [nativeDialogController blockScreenshot:(BOOL)block];
    return NULL;
}

FREObject getDeviceUniqueId(FREContext ctx, void* functionData, uint32_t argc, FREObject argv[] ){
    NativeDialogControler *nativeDialogController = functionData;
    NSString *uniqueId = [nativeDialogController getDeviceUniqueId];
    FREObject returnVal;
    const char *utf8String = [uniqueId UTF8String];
    FRENewObjectFromUTF8((uint32_t)strlen(utf8String), (const uint8_t*)utf8String, &returnVal);
    return returnVal;
}

FREObject getDisplayCutout(FREContext ctx, void* functionData, uint32_t argc, FREObject argv[] ){
    NativeDialogControler *nativeDialogController = functionData;
    NSString *jsonString = [nativeDialogController getDisplayCutout];
    FREObject returnVal;
...
FREObject setDisplayMode(FREContext ctx, void* functionData, uint32_t argc, FREObject argv[] ){
    NativeDialogControler *nativeDialogController = functionData;
    uint32_t stringLength;
    const uint8_t *displayModeStr = nil;
    const uint8_t *layoutModeStr = nil;
    NSString *displayMode = @"normal";
    NSString *layoutMode = @"cutout_never";
    
    if (argv[0] && FREGetObjectAsUTF8(argv[0], &stringLength, &displayModeStr) == FRE_OK) {
        displayMode = [NSString stringWithUTF8String:(char*)displayModeStr];
    }
    if (argv[1] && FREGetObjectAsUTF8(argv[1], &stringLength, &layoutModeStr) == FRE_OK) {
        layoutMode = [NSString stringWithUTF8String:(char*)layoutModeStr];
    }
    
    BOOL result = [nativeDialogController setDisplayMode:displayMode withLayoutMode:layoutMode];
    FREObject returnVal;
    FRENewObjectFromBool(result, &returnVal);
    return returnVal;
}

FREObject vibrate(FREContext ctx, void* functionData, uint32_t argc, FREObject argv[] ){
...
FREObject showAlertWithTitleAndMessage (FREContext ctx, void* functionData, uint32_t argc, FREObject argv[] ){
    NativeDialogControler *nativeDialogController = functionData;
    // Locals for FRE string arguments from ActionScript.
    ...
    // Convert FRE UTF-8 arguments to NSString.
    ...
    // Build NSStrings for the native alert.
...
FREObject showProgressPopup (FREContext ctx, void* functionData, uint32_t argc, FREObject argv[] )
{
    ...
    // Secondary progress (FRE argv[1]) is unused on this iOS code path.
...
void NativeDialogsContextInitializer(void* extData, const uint8_t* ctxType, FREContext ctx, uint32_t* numFunctionsToTest, const FRENamedFunction** functionsToSet)
{
    ...
    *numFunctionsToTest = numOfFun;
    ...
    if(strcmp((const char *)ctxType, LIST_DIALOG_CONTEXT)==0){
        ...
        func[3].name = (const uint8_t*) "getDisplayCutout";
        func[3].functionData = nativeDialogController;
        func[3].function = &getDisplayCutout;
        
        func[4].name = (const uint8_t*) "setDisplayMode";
        func[4].functionData = nativeDialogController;
        func[4].function = &setDisplayMode;
        
        func[5].name = (const uint8_t*) "blockScreenshot";
        func[5].functionData = nativeDialogController;
        func[5].function = &blockScreenshot;
        
        func[6].name = (const uint8_t*) "getDeviceUniqueId";
        func[6].functionData = nativeDialogController;
        func[6].function = &getDeviceUniqueId;
        
        func[7].name = (const uint8_t*) "vibrate";
        func[7].functionData = NULL;
        func[7].function = &vibrate;

        func[8].name = (const uint8_t*) "setBrightness";
        func[8].functionData = NULL;
        func[8].function = &setBrightness;
    }
