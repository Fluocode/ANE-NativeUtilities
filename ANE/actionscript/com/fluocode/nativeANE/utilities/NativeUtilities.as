...
package com.fluocode.nativeANE.utilities
{
	...
	public class NativeUtilities extends AbstractNativeUtilities
	{
		...
		/**@private*/
		/**
		 * Reports an error from static methods (no instance to dispatch). Throws Error so callers can catch or it propagates.
		...
		/**@private*/
		/**
		 * Returns the status bar height in pixels (Android) or points (iOS; value may be scaled for resolution).
		 * @return Height in pixels/points, or -1 if not available or on error.
		 * @throws Error if the call was unsuccessful and no listener handles ErrorEvent.ERROR.
		 */
		...
		/**
		 * Block Screenshot
		 * Prevents screenshots from being taken on Android (using FLAG_SECURE) and iOS (using overlay technique).
		 * Note: iOS implementation uses an overlay view that appears when app goes to background,
		 * which also happens during screenshots. This is the closest iOS equivalent to Android's FLAG_SECURE.
		 * 
		 * @param block true to block screenshots, false to allow them
		 * @throws Error if the call was unsuccessful. Or will dispatch an Error Event.ERROR if there is a listener.
		 */
		...
		/**@private*/
		/**
		 * Sets whether the status bar content (icons/text) uses a light style (for dark backgrounds) or dark style (for light backgrounds).
		 * @param light true for light content (e.g. on dark status bar), false for dark content.
		 * @throws Error if the call was unsuccessful and no listener handles ErrorEvent.ERROR.
		 */
		 ...
		 * @param color RGB color as uint (e.g. 0xFF0000 for red).
		 * @throws Error if the call was unsuccessful and no listener handles ErrorEvent.ERROR.
		 */
		...
		/**@private*/
		/**
		 * Sets whether the navigation bar content uses a light style (for dark bar) or dark style (Android only).
		 * @param light true for light content, false for dark content.
		 * @throws Error if the call was unsuccessful and no listener handles ErrorEvent.ERROR.
		 */
		...
		/**@private*/
		/**
		 * Enables or disables layout that extends into the navigation bar area (Android only).
		 * @param light true to use layout that hides/extends into navigation area, false to use default layout.
		 * @throws Error if the call was unsuccessful and no listener handles ErrorEvent.ERROR.
		 */
		...
		/**
		 * Sets the display mode and layout mode for the application.
		 * @param displayMode Display mode (DisplayMode.NORMAL, DisplayMode.FULLSCREEN, DisplayMode.IMMERSIVE)
		 * @param layoutMode Layout mode for cutouts (LayoutMode.CUTOUT_DEFAULT, LayoutMode.CUTOUT_SHORT_EDGES, LayoutMode.CUTOUT_NEVER, LayoutMode.CUTOUT_ALWAYS)
		 * @return true if successful, false otherwise
		 * @note On iOS, fullscreen mode is handled through stage.displayState. Layout mode is mainly for Android.
		 */
