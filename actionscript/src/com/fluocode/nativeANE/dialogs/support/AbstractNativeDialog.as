/**
 * Shared dialog infrastructure for the NativeUtilities ANE.
 *
 * @see com.fluocode.nativeANE.dialogs.NativeAlertDialog
 */
package com.fluocode.nativeANE.dialogs.support
{
	...
	/**
	 * Abstract base for native dialog implementations. Do not instantiate directly; use concrete dialog classes
	 * such as <code>NativeAlertDialog</code>, <code>NativeListDialog</code>, etc.
	 *
	 * @playerversion AIR 3
	 */
	public class AbstractNativeDialog extends EventDispatcher implements iNativeDialog
	{
		...
		/**
		 * Plays a native shake animation when supported by the underlying dialog.
		 *
		...
		/**
		 * Returns whether the native dialog is currently visible.
		 *
		...
		/**
		 * Dismisses the native dialog if it is visible.
		 *
		 * @param buttonIndex Button index passed to native code (e.g. which action closed the dialog)
		 *
		 * @return <code>true</code> if a dismiss request was issued
		 *
		 * @throws Error if the call fails and no <code>ErrorEvent.ERROR</code> listener is registered
		 */
		...
		/**
		 * Returns whether the runtime reports an iOS device (AIR <code>Capabilities.manufacturer</code> check).
		 *
